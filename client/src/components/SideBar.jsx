    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectUser ? "max-md:hidden": ''}`}> 
      <div className="pb-5">
         ...
         <div className='bg-[#282142] rounded-full flex items-center gap-2 py-2 px-4 mt-5'>
          ...
          <input type="text" placeholder="Search..." className='bg-transparent border-none outline-none text-white text-xs placeholder:text-[#c8c8c8] flex-1'/>
          </div>  
      ...
      <div className='flex flex-col '>
        {userDummyData.map((user,index)=>(
           <div onClick={()=>{setSelectedUser(user)}}
            key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max:text-sm' + (selectUser?._id === user._id &&' bg-[#28214]/50' ')`}>
            <img src={user?.profilePic || assets.avatar_icon} alt="Profile" className='w-[35px] aspects-[1/1] rounded-full'/>
            <div className='flex flex-col leading-5'>{user?.name}
              <p>{user.fullName}</p>{
